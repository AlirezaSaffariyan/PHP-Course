...
if (!empty($searchQuery)) {
    $whereClause = "WHERE Albums.title LIKE '%{$searchQuery}%' 
...
<head>
    ...
    <title>Advanced Album Management | Music Player</title>
...
<body>
                ...
                <div class="search-container">
                    <form action="" method="get" style="display: flex; gap: 10px;">
                        ...
                        <select name="sort">
                            ...
                            <option value="title" <?php echo $sortBy == 'title' ? 'selected' : ''; ?>>Album Title</option>
                            <option value="release_date" <?php echo $sortBy == 'release_date' ? 'selected' : ''; ?>>Release Date</option>
                        </select>
                        <select name="order">
                        ...
                        <table class="advanced-table">
                            <thead>
                                <tr>
                                    ...
                                    <th data-column="title">Album Title</th>
                                    <th>Artist</th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td onclick="openAlbumDetailsModal(<?php echo $row['id']; ?>)" style="cursor: pointer;">
                                        <?php
                                        $albumTitle = explode(" ", $row['title']);
                                        $albumTitle = array_map('ucfirst', $albumTitle);
                                        $albumTitle = implode(" ", $albumTitle);
                                        echo htmlspecialchars($albumTitle);
                                        ?>
                                    </td>
                                    ...
                                    <td>
                                        <?php if ($row['cover_image']): ?>
                                            <img src="data:image/png;base64,<?php echo base64_encode($row['cover_image']); ?>" 
                                                alt="Cover" style="width: 50px; height: 50px; object-fit: cover;">
                                        <?php else: ?>
                                            No Image
                                        <?php endif; ?>
                                    </td>
    ...
    <div id="albumDetailsModal" class="modal">
        <div class="modal-content">
            <span class="modal-close" onclick="closeModal()">&times;</span>
            <div id="modalAlbumDetails"></div>
        </div>
    </div>

    <script>
        // Function to open album details modal
        function openAlbumDetailsModal(albumId) {
            fetch(`get_album_details.php?id=${albumId}`)
                .then(response => response.text())
                .then(html => {
                    document.getElementById('modalAlbumDetails').innerHTML = html;
                    document.getElementById('albumDetailsModal').style.display = 'flex';
                })
        ...
        function closeModal() {
            document.getElementById('albumDetailsModal').style.display = 'none';
        }
...
echo '<thead>
    <tr>
        ...
        <th class="sortable-header" data-column="title">Album Title</th>
        <th>Biography</th>
        <th class="sortable-header" data-column="release_date">Release Date</th>
